...
assert.equal(run(`(()=>{combatFixture();for(let i=0;i<25;i++){offerCards();if(s.offers.length!==4||new Set(s.offers).size!==4)return false;}s.upgrades[0]=9;offerCards();return s.offers.length===4&&!s.offers.includes('holeSize')&&!('reroll' in window.holeDrift)&&!('upgrade' in window.holeDrift);})()`),true);
assert.equal(run(`(()=>{combatFixture();s.phase='build';s.bank[0]=1000;s.offers=['holeSize','miner','arrow','holeDamage'];if(place(0,1,1)||s.bank[0]!==1000||s.offers.length!==4)return false;if(!place(0,s.hole.x,s.hole.z)||s.offers.join(',')!=='miner,arrow,holeDamage'||s.bank[0]!==960)return false;if(!place(0,2,9)||s.offers.join(',')!=='arrow,holeDamage'||s.bank[0]!==920)return false;for(let i=0;i<400&&s.phase==='actions';i++)tick(1/60);if(s.phase!=='build'||s.offers.length!==2)return false;if(!place(0,6,9)||s.offers.join(',')!=='holeDamage'||s.bank[0]!==880)return false;for(let i=0;i<400&&s.phase==='actions';i++)tick(1/60);if(!place(0,s.hole.x,s.hole.z)||s.offers.length||s.bank[0]!==840)return false;updateUI();return $('cards').innerHTML===''&&$('bounceInfo').textContent==='0 CARDS LEFT'&&endTurn()&&s.offers.length===0;})()`),true);
console.log('PASS: restored prices and 150-brick cannon, four unique cards per turn, 4→3→2→1→0 consumption, no reroll/replacement, and valid unit/skill use.');
for(const side of [0,1]){
 assert.equal(run(`(()=>{combatFixture();s.side=${side};resetHole();s.phase='build';s.bank[${side}]=1000;s.offers=['holeSize','holeDamage'];const id=s.hole.id;if(!useSkill(0,s.hole.x,s.hole.z)||s.hole.id!==id||s.hole.level!==2||Math.abs(s.hole.r-.36)>1e-8)return false;if(!useSkill(0,s.hole.x,s.hole.z)||s.hole.damage!==7||s.bank[${side}]!==920)return false;s.offers=['holeSize','holeDamage'];if(!useSkill(0,s.hole.x,s.hole.z)||!useSkill(0,s.hole.x,s.hole.z)||s.bank[${side}]!==840||s.hole.damage!==9)return false;endTurn();s.phase='build';endTurn();if(s.side!==${side}||s.hole.level!==3||s.hole.damage!==9)return false;const r=s.hole.r;earn(5000,${side},null,true);if(s.hole.r!==r||s.hole.level!==3||s.hole.eaten!==5000)return false;launch(0);s.hole.z=${side}?10-r-.01:r+.01;driftStep(.01);return s.hp[1-${side}]===91&&s.hp[${side}]===100&&s.hole.r===r;})()`),true);
}
assert.equal(run(`(()=>{combatFixture();s.phase='build';s.bank[0]=40;s.upgrades[0]=9;resetHole();s.offers=['holeSize','holeDamage'];if(useSkill(0,s.hole.x,s.hole.z)||s.bank[0]!==40||s.offers.length!==2||s.hole.r!==1)return false;s.bank[0]=39;return !useSkill(1,s.hole.x,s.hole.z)&&s.damageUpgrades[0]===0&&s.offers.length===2;})()`),true);
...
console.log('PASS: flat-price upgrades persist for each side, wall damage rises by 2 points, caps/invalid drops do not spend, and swallowing never grows the hole.');
