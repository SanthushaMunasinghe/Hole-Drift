...
function collectColumn(p,i,side,growth=false){if(!p.cols[i])return 0;p.cols[i]--;p.offsets??=Array(p.cols.length).fill(0);p.offsets[i]++;earn(1,side,brickPos(p,i),growth);return 1;}
function collectPile(p,n,side){let got=0;while(got<n&&count(p)){let max=0,index=-1;for(let i=0;i<p.cols.length;i++)if(p.cols[i]>max){max=p.cols[i];index=i;}collectColumn(p,index,side);got++;}return got;}
...
function holeCollect(h,ax,az,side,main=false){
...
function bricksBlocked(x,z,w,d){return s.piles.some(p=>p.cols.some((n,i)=>{for(let l=0;l<n;l++)if(brickTouchesRect(brickPos(p,i,l),x,z,w,d))return true;return false;}));}
