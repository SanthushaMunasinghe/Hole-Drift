...
class HoleDrift3D {
 constructor(canvas){
  ...
  this.renderer.localClippingEnabled=true;this.sinkPlane=new THREE.Plane(new THREE.Vector3(0,1,0),0);
 ...
 roundedGeometry(w,h,d,r,segments=6){
  // Project subdivided cube faces around an inset box to make genuinely rounded edges.
  const geo=new THREE.BoxGeometry(w,h,d,segments,segments,segments),p=geo.attributes.position,normals=geo.attributes.normal;
 ...
 makeGold(){
  const geo=this.roundedGeometry(.3,.24,.48,.055,4),p=geo.attributes.position,colors=[];
  for(let i=0;i<p.count;i++){
  ...
  this.goldMat.clippingPlanes=[this.sinkPlane];this.goldMat.clipShadows=true;
  this.gold=new THREE.InstancedMesh(geo,this.goldMat,8192);this.gold.count=0;this.gold.castShadow=true;this.gold.receiveShadow=true;this.gold.frustumCulled=false;this.gold.instanceMatrix.setUsage(THREE.DynamicDrawUsage);this.scene.add(this.gold);
 ...
 draw(s,dt,valid,hover){
  this.drawGold(s,dt);
  const ids=new Set();for(const u of s.units){ids.add(u.id);let g=this.unitMeshes.get(u.id);if(!g){g=this.unitModel(u.type,u.side);g.traverse(child=>{if(child.isMesh){child.material.clippingPlanes=[this.sinkPlane];child.material.clipShadows=true;}});g.position.set(u.x,u.y||0,u.z);g.rotation.y=u.side?Math.PI:0;this.unitMeshes.set(u.id,g);this.scene.add(g);}g.position.set(u.x,u.y||0,u.z);g.scale.y=(u.type==='soldier'?.38:(u.type==='tank'||u.type==='holegun')?.5:1)*Math.min(1,(s.time-u.built)/.5+.05);if(u.type==='mg'&&!u.falling&&g.userData.turret){const target=(u.side?1:-1)*(u.aim||0);g.userData.turret.rotation.y+=(target-g.userData.turret.rotation.y)*Math.min(1,dt*12);}if(u.type==='wall')g.children.forEach((b,i)=>{if(i>0)b.visible=i<=u.hp;});let e=this.labelEls.get(u.id);if(!e){e=document.createElement('span');e.className='unitlabel'+(u.side?' enemy':'');this.labels.appendChild(e);this.labelEls.set(u.id,e);}e.style.display=u.type==='soldier'||u.capture?'none':'block';e.innerHTML='<i><em style="width:'+Math.max(0,u.hp/u.maxhp*100)+'%"></em></i>';const v=new THREE.Vector3(u.x,(1.12+(u.y||0))*1.65,u.z*1.08).project(this.camera),r=this.canvas.getBoundingClientRect();e.style.left=(v.x*.5+.5)*r.width+'px';e.style.top=(-v.y*.5+.5)*r.height+'px';}
